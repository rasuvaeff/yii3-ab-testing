...
Каждый вес обязан быть неотрицательным целым, и `Experiment` проверяет это в
конструкторе: дробный, строковый или отрицательный вес бросает
`InvalidExperimentException`. Ради отрицательного проверка и заведена: сумма
может по-прежнему быть `> 0`, но кумулятивная граница уходит назад, предыдущий
вариант становится недостижим, и эксперимент тихо крутит распределение,
которого никто не настраивал. Ноль остаётся валидным: он оставляет вариант
объявленным, не направляя в него трафик.

**Сумма** перепроверяется после сложения: по отдельности валидные веса, чья
сумма превышает `PHP_INT_MAX`, превращают результат `array_sum()` во float,
на котором ломается модуль бакетинга. И `Experiment`, и прямой вызов
`WeightedHashAssignmentStrategy` отвергают такую карту вместо падения в момент
назначения.
